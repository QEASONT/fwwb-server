...
class Controller:
    def __init__(self,
        ...
        """
        self.inference_engine = InferenceEngine(neural_network,
                                                use_gpu=use_gpu)
    ...
    def run_inference(self):
        ...
        while True:
            ...
            try:
                ...
                prediction_postprocessed = self.postprocess_prediction(
                    prediction)

                if #自己操控
                fun.pris(prediction_postprocessed, swi)
                if #操控别人
                send(prediction)
                if #被人操控
                ...
                # Apply callbacks
                if not all(
                        callback(prediction_postprocessed)
    ...
    def display_prediction(self, img: np.ndarray,
                           prediction_postprocessed: dict):
        # Live display
        img_augmented = self.results_display.show(img,
    ...
    def _start_inference(self):
        ...
        self.clip = np.random.randn(
            1, self.inference_engine.step_size,
    ...
    def _instantiate_video_recorders(self, img_augmented, img_raw):
        self.video_recorder = cv2.VideoWriter(
            self.path_out, 0x7634706d, self.inference_engine.fps,
            (img_augmented.shape[1], img_augmented.shape[0]))

        path_raw = self.path_out.replace('.mp4', '_raw.mp4')
        self.video_recorder_raw = cv2.VideoWriter(
            path_raw, 0x7634706d, self.inference_engine.fps,
